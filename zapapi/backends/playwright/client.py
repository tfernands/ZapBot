...
from ...errors import ChatNotFoundException, NoOpenChatException
from ...models import AuthStatus, ChatRef, ChatSummary, ChatTextMessage, HistoryPage, PollResult
...
class PlaywrightZapAPI:
    ...
    def poll(
        ...
        *,
        ...
        new_messages: list[ChatTextMessage] = []
    ...
    def _extract_visible_messages(self, chat: ChatRef) -> list[ChatTextMessage]:
        messages: list[ChatTextMessage] = []
    ...
    def _scroll_to_bottom(self) -> None:
        container = self.session.require_visible_locator(
            MESSAGE_LIST_CONTAINER_SELECTORS,
            timeout_ms=self.config.action_timeout_ms,
        )
    ...
    def _has_more_history(
        collected: list[ChatTextMessage],
        page_messages: list[ChatTextMessage],
        *,
        exhausted: bool,
        before: str | None,
    ) -> bool:
